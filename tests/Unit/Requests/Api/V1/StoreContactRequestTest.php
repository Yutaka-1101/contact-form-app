...
class StoreContactRequestTest extends TestCase
{
    ...
    /** @test */
    public function 正しいデータを受け取った場合、バリデーションを通る(): void
    {
    ...
    /** @test */
    public function 正しいデータとタグを受け取った場合、バリデーションを通る(): void
    {
    ...
    /** @test */
    public function 必須項目が空の場合、バリデーションエラーになる(): void
    {
    ...
    /** @test */
    public function 無効なメールアドレス形式の場合、バリデーションエラーになる(): void
    {
        $validator = $this->validator($this->validData([
            'email' => 'invalid-email',
        ]));
    ...
    /** @test */
    public function 無効な電話番号形式の場合、バリデーションエラーになる(): void
    {
        $validator = $this->validator($this->validData([
            'tel' => '123',
        ]));
    ...
    /** @test */
    public function お問い合わせ内容が120文字より多い場合、バリデーションエラーになる(): void
    {
        $validator = $this->validator($this->validData([
    ...
    /** @test */
    public function 不正な性別値の場合、バリデーションエラーになる(): void
    {
        $validator = $this->validator($this->validData([
            'gender' => 0,
        ]));
